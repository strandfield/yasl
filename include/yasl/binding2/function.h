...
#ifndef YASL_BINDING2_FUNCTION_H
#define YASL_BINDING2_FUNCTION_H
...
{
  return ns.newFunction(std::move(name), function_wrapper_t<decltype(F), F>::wrap)
    .returns(make_type<R>());
}
...
{
  return ns.newFunction(std::move(name), void_function_wrapper_t<decltype(F), F>::wrap);
}
...
#endif // YASL_BINDING2_FUNCTION_H
