...
void SignalMapper::activate(bool arg)
{
  ...
  invoke(this->slot, e->expose(this->receiver), { script::make_value(arg, e) });
}
...
{
  script::Engine *e = this->slot.engine();
  invoke(this->slot, e->expose(this->receiver), { script::make_value(a1, e), script::make_value(a2, e) });
}
...
script::Value connect(script::FunctionCall *c)
{
  ...
  {
    auto data = bind::BindingData::get(sender);
    data.connections.append(bind::Connection{ signal, QPointer<QObject>{receiver}, slot });
    bind::BindingData::set(sender, data);
  }
